...
mod crawl_error_tracking_tests {
    ...
    async fn insert_repository(db: &TestDatabase, repo: &Repository) -> Result<()> {
        sqlx::query(
    ...
    async fn get_repository(db: &TestDatabase, id: Uuid) -> Result<Option<Repository>> {
    ...
    async fn set_last_crawl_error(db: &TestDatabase, id: Uuid, error: Option<String>) -> Result<()> {
    ...
    async fn list_repositories(db: &TestDatabase) -> Result<Vec<Repository>> {
    ...
    /// Update a repository
    async fn update_repository(db: &TestDatabase, id: Uuid, repo: &Repository) -> Result<()> {
        sqlx::query(
    ...
    /// Delete a repository
    async fn delete_repository(db: &TestDatabase, id: Uuid) -> Result<()> {
        sqlx::query("DELETE FROM repositories WHERE id = ?").bind(id.to_string()).execute(db.pool()).await?;
        Ok(())
    }

    /// Find repositories with incomplete crawls (in_progress state)
    async fn find_incomplete_crawls(db: &TestDatabase) -> Result<Vec<Repository>> {
        let repo_rows = sqlx::query(
    ...
    async fn test_set_last_crawl_error_stores_error_message() -> Result<()> {
        ...
        insert_repository(&db, &repo).await?;

        // Set an error message
        let error_message = "Failed to clone repository: SSH key authentication failed".to_string();
        set_last_crawl_error(&db, repo.id, Some(error_message.clone())).await?;

        // Verify the error was stored
        let retrieved_repo = get_repository(&db, repo.id).await?.unwrap();
        ...
        // Clean up
        delete_repository(&db, repo.id).await?;
    ...
    async fn test_set_last_crawl_error_clears_error() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(error_message)).await?;

        // Verify error is set
        let repo_with_error = get_repository(&db, repo.id).await?.unwrap();
        assert!(repo_with_error.last_crawl_error.is_some());

        // Clear the error
        set_last_crawl_error(&db, repo.id, None).await?;

        // Verify error is cleared
        let repo_without_error = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_set_last_crawl_error_replaces_old_error() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(error1.clone())).await?;

        let repo_with_error1 = get_repository(&db, repo.id).await?.unwrap();
        ...
        set_last_crawl_error(&db, repo.id, Some(error2.clone())).await?;

        // Verify new error replaced the old one
        let repo_with_error2 = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_error_tracking_independent_across_repositories() -> Result<()> {
        ...
        insert_repository(&db, &repo1).await?;
        insert_repository(&db, &repo2).await?;
        insert_repository(&db, &repo3).await?;
        ...
        set_last_crawl_error(&db, repo1.id, Some(error1.clone())).await?;
        set_last_crawl_error(&db, repo2.id, Some(error2.clone())).await?;
        set_last_crawl_error(&db, repo3.id, error3).await?;

        // Verify each repository has correct error
        let r1 = get_repository(&db, repo1.id).await?.unwrap();
        let r2 = get_repository(&db, repo2.id).await?.unwrap();
        let r3 = get_repository(&db, repo3.id).await?.unwrap();
        ...
        // Clean up
        delete_repository(&db, repo1.id).await?;
        delete_repository(&db, repo2.id).await?;
        delete_repository(&db, repo3.id).await?;
    ...
    async fn test_set_last_crawl_error_with_long_message() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(long_error.clone())).await?;
    ...
    async fn test_set_last_crawl_error_with_special_characters() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(error_with_special_chars.clone())).await?;
    ...
    async fn test_last_crawl_error_in_list_repositories() -> Result<()> {
        ...
        insert_repository(&db, &repo_with_error).await?;
        insert_repository(&db, &repo_without_error).await?;

        let error_msg = "Crawl failed: insufficient disk space".to_string();
        set_last_crawl_error(&db, repo_with_error.id, Some(error_msg.clone())).await?;

        // List all repositories
        let all_repos = list_repositories(&db).await?;
        ...
        // Clean up
        delete_repository(&db, repo_with_error.id).await?;
        delete_repository(&db, repo_without_error.id).await?;
    ...
    async fn test_error_preserved_when_updating_repository() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(error_msg.clone())).await?;

        // Update some other fields
        repo.enabled = false;
        repo.branch = Some("develop".to_string());
        update_repository(&db, repo.id, &repo).await?;

        // Verify error is still present after update
        let updated_repo = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_error_cleared_on_successful_crawl() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(error_msg)).await?;
        ...
        // Verify error is cleared
        let repo_after_success = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_concurrent_error_updates() -> Result<()> {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                set_last_crawl_error(&db_clone, repo_id, Some(error_msg)).await
            });
        ...
        let final_repo = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_error_message_with_large_content() -> Result<()> {
        ...
        set_last_crawl_error(&db, repo.id, Some(large_error.clone())).await?;
    ...
    async fn test_updated_at_timestamp_changes_when_error_set() -> Result<()> {
        ...
        let repo_after_error = get_repository(&db, repo.id).await?.unwrap();
    ...
    async fn test_incomplete_crawls_includes_error_info() -> Result<()> {
        ...
        let incomplete = find_incomplete_crawls(&db).await?;
